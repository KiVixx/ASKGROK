...
- npm 10 或以上
- Playwright Chromium
- 可正常访问 `https://grok.com`
- 一个可登录的 Grok 账号
- 首次登录需要 headed browser，也就是有图形界面的浏览器窗口
- macOS / Linux / Windows 理论上都可以运行，但目前主要在 macOS Apple Silicon 环境测试

ASKGROK 不是官方 Grok API。它是通过 Playwright 控制真实浏览器完成自动化。因此，如果部署在没有图形界面的服务器环境，可能需要额外配置 Xvfb 或其他浏览器显示环境。
...
| --- | --- | --- |
| `PORT` | `3000` | API 服务端口 |
| `HEADLESS` | `false` | 是否以无头模式运行浏览器。首次登录通常需要设为 `false` |
| `GROK_PROFILE_DIR` | `.askgrok-profile` | 持久化浏览器资料目录 |
| `GROK_URL` | `https://grok.com` | Grok 网页地址 |
